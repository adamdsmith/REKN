...
add_weight <- TRUE
if (add_weight) {
  stopifnot(exists("rekn_dep"))
  rekn_map_dat <- left_join(rekn_map_dat, select(rekn_dep, motusTagID, wt_g), by = "motusTagID")
}
...
tags <- sort(unique(rekn$motusTagID))

# Get band/flag resightings
fn <- list.files("Data", "Smith_all_rekn", full = TRUE)
...
bborg <- readxl::read_excel("Data/REKN SC resight bandedbirds April 2020.xlsx") %>%
...
resight <- bind_rows(bbl, bborg) %>%
  mutate(lat = round(lat, 3),
         lon = round(lon, 3)) %>%
  distinct() %>%
  inner_join(rekn_dep, by = "band_num") %>%
...
pdf(paste0("Output/REKN_detection_maps_vetted_", Sys.Date(), ".pdf"), width = 10, height = 8.5)
for (i in tags) {
...
dev.off()
system(paste('open', shQuote(paste0("Output/REKN_detection_maps_vetted_", Sys.Date(), ".pdf"))))
