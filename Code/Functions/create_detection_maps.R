create_detection_maps <- function(det_data, outfile = "detection_maps.pdf",
  ...
  tmp_map_dat <- det_data %>%
    select(mfgID, motusTagID, label, dep_lat = tagDepLat, dep_lon = tagDepLon, tagDeployStart,
           date, recvDeployID:recvSiteName, antBin, age, sex) %>%
    distinct()
  ...
  timez <- st_read("Resources/geodata/ne_10m_time_zones.shp", quiet = TRUE) %>%
  ...
  for (i in tags) {
    ...
    p <- p +
      ...
      coord_sf(xlim = lon_rng + c(-2, 2),
               ylim = lat_rng + c(-2, 2),
               expand = FALSE) +
      ggtitle(gsub("NA", "Unk", unique(paste(tmp_map$label, tmp_map$age, tmp_map$sex, sep = ", ")))) + 
      guides(shape = "none") +
      labs(x = NULL, y = NULL) + 
