...
if (update_detections) {
  rekn <- compile_detections(projects, dir = list("~/FWS_Projects/Motus/Stations/Data", "Data/Motus_raw", "Data/Motus_raw"),
                             minRunLen = 3,
                             speciesID = 4670, tagProjID = as.list(projects))
  rekn4 <- filter(rekn, runLen >= 4)
  rekn3 <- filter(rekn, runLen == 3, longRun == 3)
  rekn <- bind_rows(rekn4, rekn3)
...
rekn_dep <- readxl::read_excel("Data/tag-deployments.xlsx") %>%
  mutate(marker = paste(markerType, markerNumber, sep = ": "),
         marker = ifelse(marker == "NA: NA", "", marker),
         age = ifelse(is.na(age), "", age),
         band_num = as.numeric(gsub("-", "", bandNumber)),
         bdate = as_date(dtStart),
         state = ifelse(latitude < 30, "FL", "SC")) %>%
  select(motusTagID = tagID, state, bdate, band_num, marker, age, wt_g = weight)

message("REKN weights at capture; n = ", sum(!is.na(rekn_dep$wt_g)))
print(summary(rekn_dep$wt_g)) 
message("Standard deviation: ", round(sd(rekn_dep$wt_g, na.rm = T), 1), " g")
rekn <- left_join(rekn, rekn_dep, by = "motusTagID")

# Create inspection sheet for logging accept/reject status of marginal detections
if (!file.exists("Data/rekn_inspection_log.csv")) create_inspection_sheet(rekn, out_dir = "Data")
...
if (reproduce_pdfs) {
  ...
  resight <- get_resights(rekn_dep)

  create_detection_profiles(rekn, out_pdf = paste0("Output/REKN_detection_validation_", Sys.Date(), ".pdf"))
  ...
  pdf(paste0("Output/REKN_detection_maps_", Sys.Date(), ".pdf"), width = 10, height = 8.5)
  for (i in tags) {
    tmp_map <- filter(rekn_map_dat, motusTagID == i)
    tmp_det <- filter(rekn_det_dat, motusTagID == i) %>%
  ...
  dev.off()
  system(paste('open', shQuote(paste0("Output/REKN_detection_maps_", Sys.Date(), ".pdf"))))
}


if (!file.exists("Data/Derived/rekn_vetted.rds")) {
  # Filter records based on inspection
  failed_insp <- readr::read_csv("Data/rekn_inspection_log.csv") %>%
    filter(!keep) %>%
    pull(runID) %>% unique()
  rekn <- filter(rekn, !(runID %in% failed_insp))
  ...
  saveRDS(rekn, "Data/Derived/rekn_vetted.rds")
} else rekn <- readRDS("Data/Derived/rekn_vetted.rds")

# Create vetted detection maps
if (reproduce_pdfs) source("Code/create_vetted_detection_maps.R")
